...
| 피측정자 | 정확도 | n | 조건 |
|---|---|---|---|
| 규칙 엔진 (h° 62, 라운드 1) | **63.2%** | 76 | 확신 케이스 한정 (선택 편향), 95% CI 약 ±11%p |
| 규칙 엔진 (h° 62, 라운드 2) | 69.1% | 55 | 동일 조건, ±13%p — 라운드 간 변동 자체가 표본 규모의 기록 |
| **재보정 엔진 (b\*+L\*, 라운드 2)** | **72.7%** | 55 | 동일 조건. 구엔진과 통계적 동률(우세 경향) — ADR-010에 A/B 상세 |
...
- [x] ~~특징 조합 재설계 검토~~ → 실행됨 (ADR-010): 언더톤 축을 b\*+L\*로 재보정, 라운드 2(55장) A/B에서 유지 확정
- [ ] 검증셋 확충 — "우세 경향"을 "개선 확정"으로 올리려면 유의성이 필요하고, 그건 더 큰 검증셋(누적 300장+)의 몫
